...
def get_session_details(conn, source_session_id):
    rows = conn.execute(
        "SELECT id, role, content FROM conversations WHERE session_id = ? ORDER BY id LIMIT 50", (
            source_session_id,)
    ).fetchall()
    return rows
...
def trigger_consolidation():
    ...
    ids = get_ids(conn,
                  unconsolidated)
    session_rows = get_session_details(conn, unconsolidated)
    ...
    if reply is None:
        ...
        update_consolidated_status(
            conn, unconsolidated, ids)  # still mark it done
    else:
        fact_data = hash_converter(reply)
        save_facts(conn, unconsolidated, fact_data)
        update_consolidated_status(
            conn, unconsolidated, ids)
        if check_saved_facts(conn, unconsolidated):
            del_raw_conversations(conn, unconsolidated)
